...
def run(opt_package="tao_lmvm",
    ...
    for level in range(max_level + 1):
        ...
        convert_options = {"riesz_representation": ctrl_inner_product}
        ...
        if opt_package == "tao_lmvm":
            ...
            def cb(x):
                counter["nit"] += 1
            start_time = time.time()
            f_opt = minimize(rf, tol=1.0e-6, method="L-BFGS-B", options={"maxiter": 15000, 'maxcor': 10}, callback = cb, derivative_options={"riesz_representation":ctrl_inner_product})
            runtime = time.time() - start_time
            iter = counter["nit"]
